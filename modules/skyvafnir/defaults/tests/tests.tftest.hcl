...
run "verify_base" {
  ...
  # resource_name
  assert {
    condition = output.resource_name == join("-", [var.org_code, var.resource_abbreviation, var.instance, var.tier])
    error_message =  "Output resource_name is incorrect"
  }

  # resource_name_template
  assert {
    condition = output.resource_name_template == join("-", [var.org_code, "%s", var.instance, var.tier])
